...
function showFilesUser() {
	...
	if( $row = $myDB->query( "SELECT * FROM  `files` LIMIT 0 , 25" ) ) {
		...
		echo "<table width='70%' border='1'><tr align='center' bgcolor='#ffcc00'><td>&nbsp</td><td><b>id</b></td><td><b>Имя файла</b></td><td><b>Дата загрузки</b></td><td>&nbsp</td></tr><form method='post'>";
		$fileUrl = 'http://'.$_SERVER['HTTP_HOST'].dirname($_SERVER['PHP_SELF']).'/upload/';
		foreach( $row as $file ) {
		...
		echo "</table><table width='70%'><tr><td><input type='submit' value='удалить отмеченные'></form></td></tr></table>";
	}
}

function showUploadForm() {
    echo "<br><hr><br><form enctype='multipart/form-data' method='post'><input type='file' name='ufile'><br><br><input type='submit' value='Загрузить файл'></form>";
}
